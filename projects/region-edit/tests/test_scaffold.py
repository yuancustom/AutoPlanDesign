...
class ScaffoldTest(unittest.TestCase):
    def test_capabilities_are_explicit(self):
        ...
        r = subprocess.run([sys.executable, "-m", "autoplan_region", "--capabilities"], env=env, capture_output=True, text=True, check=True)
        result = json.loads(r.stdout)
        self.assertEqual(result["project"], "P2")
